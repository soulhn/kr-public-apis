# work24_poa68_ncs20_dynamic_shard.py
...
#국가인적자원개발컨소시엄_202401_202508_NCS정보통신_4,225건
# ■ 실행/출력
...
# HRDPOA68: 컨소시엄/지역맞춤/산업계주도 등
BASE_URL = "https://hrd.work24.go.kr/jsp/HRDP/HRDPO00/HRDPOA68/HRDPOA68_1.jsp"
BASE_PARAMS = {
    "authKey": AUTH_KEY,
    "returnType": "XML",
    "outType": "1",
    ...
    "sort": "ASC",
    "sortCol": "2",       # 1:기관명, 2:시작일, 3:취업률, 4:만족도
    "srchNcs1": "20",     # ✅ NCS 1차: 정보통신
    # 전체 유형 수집을 원하므로 crseTracseSe 미등록(필요 시 "C0042,..." 지정)
}
...
def main():
    ...
    if not month_frames or all(len(df)==0 for df in month_frames):
        ...
        out_xlsx = f"{ts}_poa68_ncs20.xlsx"
    ...
    try:
        ...
        # xlsxwriter 없으면 openpyxl로 자동 폴백
